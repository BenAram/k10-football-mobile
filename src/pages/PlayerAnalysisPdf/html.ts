...
function createHtml(inputs: Array<Input>, values: InputsValue, positionBalls: Array<PositionBall>, positionHitmaps: Array<PositionHitmap>, accurateAssists: Array<AccurateAssists>, player: Player, match: Match): string {
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .field {
            width: ${width * 0.25 * 0.8}px;
            height: ${height * 0.9 * 0.25 * 0.8}px;

            background-image: url(https://cdn.discordapp.com/attachments/851085431042998293/861955182853095434/vertical-field.png);
...
<body>
    ...
    <main>
        ...
        <section id="section-footer">
            ...
            <div class="field-container" id="field-container2">
                <p>Accurate Assists</p>
                <div class="field">
                    {accurate-assists}
                </div>
            </div>
...
positionBalls.map(positionBall => {
    const ball = 'https://cdn.discordapp.com/attachments/856988081364926515/857799304794210334/ball.png';
    const goldenBall = 'https://cdn.discordapp.com/attachments/851085431042998293/861949270164504576/golden-ball.png';
    let calc = 0.03;
    if (positionBall.x > (width * 0.45) / 2) {
        calc = 0;
    }
    $balls += `
        <img
            src="${positionBall.golden ? goldenBall : ball}"
            class="${positionBall.golden ? 'golden-ball' : 'ball'}"
            style="transform: translate(${positionBall.x - (width * calc)}px, ${positionBall.y}px)"
        />
    `
})
...
let $accurateAssistsArrows = '';
const ball = 'https://cdn.discordapp.com/attachments/856988081364926515/857799304794210334/ball.png';
accurateAssists.forEach(accurateAssist => {
    $accurateAssistsArrows += `
<div class="field-arrow" style="width: ${accurateAssist.width}px; transform: translate(${accurateAssist.translateX}px, ${accurateAssist.translateY}px) rotate(${accurateAssist.angle}deg) translateX(${accurateAssist.width / 2}px)">
    <img src="${ball}" style="transform: translate(${accurateAssist.width * 0.5}px, -13.5px)"/>
</div>
`
})
...
const urlChart = `https://quickchart.io/chart?bkg=transparent&c=${JSON.stringify(finalObject)}`
    ...
    html = html.replace('{balls}', $balls);
    html = html.replace('{chart}', encodeURI(urlChart));
    html = html.replace('{hit-map}', $hitmap);
    html = html.replace('{accurate-assists}', $accurateAssistsArrows);
