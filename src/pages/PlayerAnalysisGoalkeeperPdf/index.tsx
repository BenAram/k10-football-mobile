...
import { leftInputs, rightInputs } from '../PlayerAnalysisGoalKeeper/inputs';
...
function PlayerAnalysisPdf() {
    ...
    async function handleDownload() {
        try  {
            ...
            let html = htmlPdf;
            html = html.replace('{bottom-hitmap-hand}', `${height * 0.315}`)
    ...
    useEffect(()  => {
        async function getUri() {
            try {
                ...
                let newHtml = createHtml(inputs, params.playerStats, params.positionHands, params.positionHitmaps, params.player, params.match);
                setHtml(newHtml.replace('{bottom-hitmap-hand}', `-${height * 0.045}`));
                setHtmlPdf(newHtml);
                setReady(true);
            } catch(err) {
                alert(err);
